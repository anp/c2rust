        ...
        if expected_ty & TYPE != 0 {
            ...
            match node.tag {
                ...
                ASTEntryTag::TagFunctionDecl if expected_ty & OTHER_DECL != 0 => {
                    ...
                    let is_extern = node.extras[1]
                        .as_boolean()
                        .expect("Expected to find visibility");
                    let is_inline = node.extras[2]
                        .as_boolean()
                ...
                ASTEntryTag::TagVarDecl if expected_ty & VAR_DECL != 0 => {
                    ...
                    let is_static = node.extras[1]
                        .as_boolean()
                        .expect("Expected to find duration");
                    let is_extern = node.extras[2]
                        .as_boolean()
                        .expect("Expected to find visibility");
                    let is_defn = node.extras[3]
                        .as_boolean()
                        .expect("Expected to find whether decl is definition");
                    assert!(
                        if is_extern { is_static } else { true },
                        "Something cannot be extern without also being static"
                    );
